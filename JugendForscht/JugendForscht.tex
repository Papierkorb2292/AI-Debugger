...
\section {Komprimierung und Verarbeitung der Konversation}
Um zu verstehen, wie die Kommunikation zwischen dem Debug Adapter (DA) und dem KI-Debugger-Dienst (KDD) funktioniert, müssen deren Komprimierung und Verarbeitung genauer betrachtet werden.

Zunächst werden die Daten des DA in für den KDD verständliche Begriffe übersetzt. In der aktuellen Version sind dies: 'BREAKPOINT', 'LINE', 'CONTINUE', 'STEP', 'STEPINTO', 'STEPOUT', 'VARIABLE', 'ERROR' und 'CAUSE'.

Anschließend werden diese 'AI Commands' in eine sprachliche Form gebracht, die die Kommunikation mit dem LLM (Large Language Model) ermöglicht. Dies geschieht durch festgelegte Sätze. Zum Beispiel entspricht das 'AI Command BREAKPOINT' dem Satz: 'You can set breakpoints in the code by starting your message with \$\{id\} followed by the line number and the file name.' Dieses Prinzip wird auf jedes 'AI Command' angewendet.

Nun, da die Befehle für die KI vorliegen, werden diese übermittelt und die Antworten empfangen. Die jeweiligen Antworten sind so formatiert, dass sie ohne großen Aufwand weiterverarbeitet werden können.
